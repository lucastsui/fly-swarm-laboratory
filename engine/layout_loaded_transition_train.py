...
Explicit new transition-augmented Jacobian algorithm, not Adam resume.
...
from .layout_loaded_transition_probe import active_constraints
...
ALGORITHM='one-verified-loaded-transition-pickup-step-v1'
...
def apply_verified_step(model,bank,published,original):
    ...
    if (active!=published.get('activeLoadedFrames') or original['parentParameterHash']!=before
            or original['windowIndexes']!=published['windowIndexes']):
        raise ValueError('Original rejected-probe transition constraints required')
    ...
    try:
        ...
        committed=True
        return {'beforeHash':before,'afterHash':model.checkpoint_hash(),'backtrack':chosen['backtrack'],'activeLoadedFrames':active,
...
def main(args):
    ...
    rejected=json.loads((args.rejected_source/'result.json').read_text())
    if not r.get('oldTrainingAndServiceGuardsUnchanged'):raise ValueError('Original transition probe required')
    ...
    for p in (args.candidate,args.cache/'cache.npz',args.cache/'manifest.json',args.dataset/'manifest.json',
              args.rejected_source/'manifest.json',args.rejected_source/'result.json'):
    ...
    manifest={'algorithm':ALGORITHM,'maximumUpdates':1,'canonicalLearner':'Spark1 only','parentAdamConsumed':False,
        'optimizerState':'New stateless transition-augmented Jacobian solve; deterministic original selections; no RNG',
        'parentParameterHash':model.checkpoint_hash(),'fixedHash':model.fixed_hash,'inputFileHashes':hashes,'sourceHashes':sources,
        'exactReLUDerivative':True,'gradientFrames':96,'prefixAgeUpdates':0,'jacobianOutputs':18+len(r['activeLoadedFrames']),
        'activeLoadedFrames':r['activeLoadedFrames'],'motionBudget':list(MOTION_BUDGET),
    ...
    try:
        row=apply_verified_step(model,bank,r,rejected)
...
if __name__=='__main__':
    ...
    for n in ('root','candidate','cache','dataset','probe','rejected-source','out'):p.add_argument('--'+n,type=Path,required=True)
    main(p.parse_args())
